...
%eq24 = - Fbi*rh - Mb*rb + Fk*rd - Fby*rh == 0;   ÖVERFLÖDIG                        %Momentjämvikt kring x
...
figure (2)
plot(ix, Ty)
title(['Tvärkraft i y-riktning Lastfall ' [LF]])
xlabel('Längd [m]')
ylabel('Kraft [N]')

% Tvärkraft i z-riktning
Tz1 = -Vbi;
Tz2 = -Vbi + Vli;
Tz3 = -Vbi + Vli + Vb;
Tz4 =  Tz3;
...
figure (3)
plot(ix, Tz)
title(['Tvärkraft i z-riktning Lastfall ' [LF]])
xlabel('Längd [m]')
ylabel('Kraft [N]')
...
maxMy = max(abs(My));               %maxvärde My
maxMz = max(abs(Mz));               %maxvärde Mz
maxM  = sqrt(maxMy^2 + maxMy^2);    %Sammansatt böjmoment enligt pythagoras

maxTy = max(abs(Ty));               %maxvärde Ty
maxTz = max(abs(Tz));               %maxvärde Tz
maxT  = sqrt(maxTy^2 + maxTz^2);    %Sammansatt tvärkraft enligt pythagoras

Sigma_s = 310*10^6;            %Sträckgräns 310Mpa för Stål SS165001 
Sigma_eff = Sigma_s/ns;        %Effektivspänning (ns given säkerhetsfaktor)
...
%qn4 = T_max - Mv/Wv == 0;               
%qn5 = Sigma_eff - sqrt(Sigma_max^2+3*(T_max)^2) == 0; %Effektivspänning
...
Wv = (pi*z^3)/16            %Böjmotstånd  Se FS 6.9
Wb = (pi*z^3)/32            %Vridmotstånd Se FS 6.78

Mtot = sqrt(Mxy.^2 + Mxz.^2) %vektoraddera och sätt i en vektor
Sigma_max = Mtot/Wb
t_max = Mx/Wv %Vridmoment/Vridmotstånd
Sigma_vm = []
